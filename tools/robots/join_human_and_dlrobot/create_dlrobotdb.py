...
class TDeclaratorDBSqueezes:
    ...
    def _copy_human_merges(self, sha256_to_human_file, human_persons, dlrobot_db):
        in_cursor = dlrobot_db.db_connection.cursor()
        in_cursor.execute(
            """select
                    id, 
                    original_fio,
            ...
            """.format(dlrobot_db.section_table_name))
        dlrobot_section_id_to_person_id = dict()
        used_stable_ids = set()
        for dlrobot_section_id, fio, income_year, declarant_income, source_file_sha256, document_id in in_cursor:
        ...
        print("there are {} human merged sections that were not found in dlrobot db (smart_parser parsed files with errors?)".format(
            len(missing_set)))

        in_cursor = dlrobot_db.db_connection.cursor()
        mergings_count = 0
        for id, person_id in dlrobot_section_id_to_person_id.items():
            in_cursor.execute("update {} set person_id = {} where id={}".format(
                    dlrobot_db.section_table_name, person_id, id))
            mergings_count += 1
        dlrobot_db.db_connection.commit()
        in_cursor.close()
...
class TDlRobotDB:
    ...
    def create_section_tables(self):
        out_cursor = self.db_connection.cursor()
        out_cursor.execute("drop table if exists {};".format(self.section_table_name))
        out_cursor.execute(""" 
                    create table {} 
    ...
    def import_json(self, office_name, filepath, source_file_sha256, document_id):
        ...
        for domain in office_domains:
            ...
            for file_path, source_file_sha256, document_id in jsons_to_import:
                try:
                    ...
                    dlrobot_db.import_json(office_name, file_path, source_file_sha256, document_id)
