...
def check_mkrf_documents(link_info):
    text = link_info.Text.strip(' \n\t\r').strip('"').lower()
    if text.find("сведения") == -1:
        return False
    if link_info.Target is  not None:
        return link_info.Target.lower().find("mkrf.ru/documents") != -1
...
if __name__ == "__main__":
    ...
    find_links_for_all_websites(offices, "declarations_div_pages", "declarations_div_pages2", check_mkrf_documents)
