...
python3 ../../scripts/cloud/dlrobot_worker.py start --working-folder $WORKDIR --timeout-before-next-task 2&
WORKER_PID=$!
sleep 3
kill -0 $WORKER_PID
if [ $?  != 0 ]; then
...
python3 ../../scripts/cloud/dlrobot_central.py --result-folder  ${RESULT_FOLDER} --disable-smart-parser-cache --central-heart-rate 1s &
CENTRAL_PID=$!
sleep 1
