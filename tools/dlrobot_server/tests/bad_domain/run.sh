...
python3 ../../dlrobot_central.py --tries-count 2 --server-address ${WEB_ADDR} --input-folder input_projects --result-folder  ${RESULT_FOLDER}  --disable-smart-parser-cache&
WEB_SERVER_PID=$!
sleep 1
...
function run_worker() {
  ...
  python3 ../../dlrobot_worker.py run_once --server-address ${WEB_ADDR} --tmp-folder ${WORKER_DIR}
