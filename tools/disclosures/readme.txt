...
#1. получение двух репозиториев:
 ...
 export SCRIPT_FOLDER=~/smart_parser/tools/robots/join_human_and_dlrobot
...
#6.  слияние по файлам dlrobot и declarator, получение dlrobot_human.json
    ...
    (возможно там аргументы изменились)

#7.  создание базы disclosures
    cd $DISCLOSURES_FOlDER
    cat create_disclosures_db.sql | sudo mysql
    python manage.py makemigrations
    python manage.py migratr
    cd $DLROBOT_FOLDER

#8.  Импорт json в dislosures_db
   cd $DLROBOT_FOLDER
   python3 $DISCLOSURES_FOlDER/manage.py import_json --smart-parser-human-json-folder human_smart_parser_jsons  --dlrobot-human dlrobot_human.json  --process-count 4
