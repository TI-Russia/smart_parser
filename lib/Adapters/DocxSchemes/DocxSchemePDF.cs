...
{
    class DocxSchemePDF : IDocxScheme
    {
        ...
        public override bool CanProcess(WordprocessingDocument document)
        {
            ...
            if (titles.Count % 4 != 0 || titles.Count == 0)
                return false;

            if (!titles[0].InnerText.Contains("Сведения о доходах"))
                return false;
            if (!titles[1].InnerText.Contains("Сведения о расходах"))
                return false;
            if (!titles[2].InnerText.Contains("Сведения об имуществе"))
                return false;
            if (!titles[3].InnerText.Contains("Сведения об обязательствах"))
                return false;
